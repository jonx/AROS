...
char *readline(FILE *descf)
{
    ...
    if (!g_data || g_dpos >= g_dlen)
        return NULL;

    start = g_dpos;
    while (g_dpos < g_dlen && g_data[g_dpos] != '\n')
        g_dpos++;
    end = g_dpos;
    if (g_dpos < g_dlen)
        g_dpos++;                           /* step over the '\n' */
    if (end > start && g_data[end - 1] == '\r')
        end--;                              /* strip CR of a CRLF */

    len = end - start;
    if (slen < (unsigned int)len + 1)
    {
        if (line) free(line);
        slen = (unsigned int)len + 256;
        line = malloc(slen);
        if (!line) { slen = 0; return NULL; }
    }
    CopyMem(g_data + start, line, len);
    line[len] = '\0';
    lineno++;
    return line;
}
...
BOOL parseKeyDescriptor(struct config *cfg)
{
    ...
    if (fib && ExamineFH(fh, fib) && fib->fib_Size > 0)
    {
        g_dlen = (IPTR)fib->fib_Size;
        g_data = malloc(g_dlen + 1);
        if (g_data && Read(fh, g_data, g_dlen) == (LONG)g_dlen)
        {
            g_data[g_dlen] = '\0';
            g_dpos = 0;
            NEWLIST(&cfg->KeyDesc);
            retval = processDescriptor(cfg, NULL);
        }
        if (g_data) { free(g_data); g_data = NULL; }
    }
    if (fib)

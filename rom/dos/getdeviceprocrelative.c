/*
    ...
    Desc: GetDeviceProcRelative() - Find the filesystem for a path.
*/
...
static struct DevProc *deviceproc_internal(struct DosLibrary *DOSBase, BPTR baselock, CONST_STRPTR name, struct DevProc *dp);
...
#include <proto/dos.h>
        ...
        AROS_LH3(struct DevProc *, GetDeviceProcRelative,

/*  SYNOPSIS */
        AROS_LHA(BPTR, lock, D1),
        AROS_LHA(CONST_STRPTR, name, D2),
        AROS_LHA(struct DevProc *, dp, D3),

/*  LOCATION */
        struct DosLibrary *, DOSBase, 229, Dos)

/*  FUNCTION
        GetDeviceProcRelative() will search for the filesystem handler which
        you should send a command to for a specific path.

        By calling GetDeviceProcRelative() multiple times, the caller will be
        able to handle multi-assign paths.

        The first call to GetDeviceProcRelative() should have the |dp|
        parameter as NULL.

    INPUTS
        lock - existing lock to compute filename from
        name - Name of the object to find.
        dp   - Previous result of GetDeviceProcRelative() or NULL.
...
*****************************************************************************/
{
    ...
    D(bug("[GetDeviceProcRelative] lock=0x%p name='%s' dp=0x%p\n", lock, name, dp));
    dp2 = deviceproc_internal(DOSBase, lock, name, dp);

#if DEBUG
    bug("[GetDeviceProcRelative] = 0x%p", dp2);
    if (dp2)
...
static struct DevProc *deviceproc_internal(struct DosLibrary *DOSBase, BPTR baselock, CONST_STRPTR name, struct DevProc *dp)
{
    ...
    if (dl->dol_Type == DLT_DEVICE || dl->dol_Type == DLT_VOLUME)
    {
        ...
        if (dl->dol_Type == DLT_DEVICE)
        {
            ...
            if (!newhandler)
            {
                D(bug("[GetDeviceProcRelative] Accessing device '%b', path='%s'\n", dl->dol_Name, origname));
        ...
        else
        {
            ...
            {
                D(bug("[GetDeviceProcRelative] Accessing offline volume '%b'\n", dl->dol_Name));
    ...
    /* otherwise we're finding the next */
        ...
        /* find our current lock (ie the one we returned last time), then skip
         * past all consecutive entries with that same lock */
        for (; al != NULL && al->al_Lock != dp->dvp_Lock; al = al->al_Next);
        al = deviceproc_nextassignlock(al, dp->dvp_Lock);

        /* if we didn't find it, or there's none after it, then we've run out */
        if (al == NULL) {
            UnLockDosList(LDF_ALL | LDF_READ);
            FreeMem(dp, sizeof(struct DevProc));
